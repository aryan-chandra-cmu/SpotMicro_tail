...
"""
Offline Gait trajectory test and visualization.

Instantiates all four leg objects (without hardware) and Trot controller,
generates a dynamic test trajectory, and plots per leg:
...
def simulate_leg(leg, trot, leg_name, total_time, dt, num_steps, current_forward_speed):
    """
    Run the offline gait simulation for a single leg.
    """
    ...
    for step in range(num_steps):
        ...
        # Get foot position from parametric gait trajectory
...
def main():
    ...
    for leg_name, leg in legs.items():
        data = simulate_leg(leg, trot, leg_name, total_time, dt, num_steps, current_forward_speed)
        plot_leg(data, f"{leg_titles[leg_name]} (Gait: {current_forward_speed} m/s)")
