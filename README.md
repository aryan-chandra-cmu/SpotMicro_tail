...
Robot: 
The quadruped robot model was created and modified to represent the SpotMicro platform with the intended tail and payload configuration, forming the foundation for subsequent locomotion and stability studies. This model was then connected to a MuJoCo-based Python workflow in which the robot could be spawned directly into a controlled standing posture using joint-wise position targets for all four shoulders, upper legs, and lower legs. In particular, the stand/spawn script initializes the robot in a physically meaningful configuration by writing the desired pose into the simulation state and then commanding the actuators to hold that posture, ensuring that the model starts from a stable and repeatable configuration before walking experiments begin. This step was important because it established a reliable bridge between the modified robot description and the low-level joint-space control framework needed for later gait testing and tail-related analysis. In parallel, a left-right leg kinematic control approach was developed to organize joint actuation leg-by-leg, so that each side of the robot could be treated systematically during pose generation, gait sequencing, and future inverse-kinematics-based motion refinement.

Torque Logging:
After establishing the robot model and standing configuration, an open-loop trot gait was developed and visualized using Python and MuJoCo. The walking controller applies a diagonal trot pattern in which front-left/rear-right and front-right/rear-left leg pairs alternate in phase, while the hip, knee, and shoulder joints are driven around the standing pose using time-varying trajectories. The gait script explicitly distinguishes swing and stance phases, increases knee flexion during swing for foot clearance, and adds small coordinated shoulder motions for improved symmetry and clearance, allowing the robot’s locomotion behavior to be visualized directly in simulation. To support hardware feasibility and control validation, the same gait framework was extended with a torque-logging script that records actuator torques over time for all shoulder, leg, and foot joints while the robot walks. This provided quantitative evidence (Figure 5) that the open-loop gait was not only visually executable in simulation, but also measurable at the joint-actuation level, making it possible to assess whether the motor demands remained within a practical range for the proposed tailed SpotMicro system.
